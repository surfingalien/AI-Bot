...
```bash
npm test                  # 195 tests, no network required
...
A spinner appears because the interface has nothing true to say yet. The fix is
not a nicer spinner — it is arranging for something true to exist at submit.

Every turn now opens with an acknowledgement built from the utterance alone:

```
you    what do you make of https://www.reuters.com/markets/ right now
desk   Reading reuters.com…                     ← at ~0 ms, no round trip
desk   Reuters is reporting a steady session.   ← replaces it on first token
```

It costs no network call, no model call, and no I/O of any kind — which is both
why it can appear instantly and why it can never be wrong about something it had
to go and find out. A URL is named by its host, a ticker by its symbol, a
booking by what it is. Capitalised English is not mistaken for a ticker, because
"Pulling THE…" is worse than saying nothing specific.

It is a placeholder, not an answer: the first streamed token replaces it.

### Waking up

The desk greets on boot from what the browser knows, which is nothing about what
happened while the tab was shut — and that is the only interesting thing to say
on waking. Entering the desk now asks the server what it has been doing:

```
Good morning, Operator. 2 goals armed. Nothing fired while you were away.
```

Spoken as well as shown, when speech is on. Only the last twelve hours count as
news, and with no goals armed it does not claim a quiet night — nothing could
have fired, which is not the same as nothing happening.
...
test/                  195 tests, no network required
```
